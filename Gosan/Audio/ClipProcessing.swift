...
enum ClipProcessing {
    ...
    /// Render a pitch-shifted copy of a clip's segment (length preserved). semitones in ±.
    static func pitchShift(url: URL, offset: TimeInterval, duration: TimeInterval,
                           semitones: Float, outputDir: URL) -> URL? {
        guard semitones != 0,
        ...
        let outFrames = AVAudioFramePosition(input.frameLength)   // rate 1 → same length
        let out = outputDir.appendingPathComponent("\(UUID().uuidString)-pitched.caf")
        guard let outFile = try? AVAudioFile(forWriting: out, settings: format.settings),
